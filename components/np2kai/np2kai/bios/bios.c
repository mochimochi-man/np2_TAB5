...
void bios_initialize(void) {
	...
	if(np2cfg.usebios){
		...
		if (fh == FILEH_INVALID) {
			/* Nothing chosen in the menu, so use the compatible BIOS built
			   into the firmware. Deliberately ahead of a bios.rom sitting on
			   the card: a stock unit has to come up with no card contents at
			   all (the M5Burner build ships an image and nothing else), and a
			   dumped ROM is then selected explicitly rather than by being
			   present. dosio_sd.cpp serves this path out of flash. */
