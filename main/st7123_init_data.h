// ST7123 panel initialisation data for the M5Stack Tab5.
//
// Copied verbatim from the Tab5 BSP
// (managed_components/espressif__m5stack_tab5/priv_include/disp_init_data_1.h,
// Copyright 2026 Espressif Systems, Apache-2.0). It lives here because that
// header is in the component's priv_include and is not on any include path,
// and because panel_tab5.cpp does the display bring-up itself rather than
// through bsp_display_new().
//
// These are the values Espressif and M5 ship for a real ST7123 Tab5, which is
// the point: no unit with this panel has ever run this firmware, so the whole
// ST7123 path follows the vendor driver and the vendor data rather than the
// hand-rolled sequence that the ST7121 needed.
//
