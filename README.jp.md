...
同時に使用するには、AC アダプタ付きセルフバスパワー対応ハブが必要です。
...
Tab5 の USB-A ポートに接続した GreaseWeazle 経由で、ドライブ実機から PC-9801 用
2DD / 2HD フォーマットのディスクイメージ（`.NFD`）を SD カードに吸い出します。
**コピープロテクトには対応しません。**
...
- M5Stack Tab5 は、最新ビルド現在、液晶に **ST7123 / ST7121 / ILI9881C** の
  いずれかが使用されています。開発に用いた Tab5 は **ST7121** を使用しており、
  **他のモジュールでの動作は確認できていません。** 画面に何も表示されないなどの
  不具合がありましたら、X アカウント **[@calorie0](https://x.com/calorie0)** まで
  お知らせいただけると幸いです（ただし、対応を保証するものではありません）。
